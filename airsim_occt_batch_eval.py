...
def build_command(args, method: str, road_index: int) -> list[str]:
...
def main():
    ...
    for method in args.methods:
        for road_index in args.roads:
            cmd = build_command(args, method=method, road_index=road_index)
            print(f"\n[BATCH] method={method} road={road_index}")
            print("[BATCH] command:", " ".join(cmd))
            result = subprocess.run(cmd, cwd=Path(__file__).resolve().parent)
            if result.returncode != 0:
                failures.append((method, road_index, result.returncode))
                print(f"[BATCH] FAILED method={method} road={road_index} returncode={result.returncode}")
                if not args.continue_on_error:
                    raise SystemExit(result.returncode)
            else:
                print(f"[BATCH] OK method={method} road={road_index}")

    if failures:
        print("\n[BATCH] summary: failures detected")
        for method, road_index, returncode in failures:
            print(f"  method={method} road={road_index} returncode={returncode}")
        raise SystemExit(1)
