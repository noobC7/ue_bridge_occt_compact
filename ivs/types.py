...
class TrackControls(MsgpackMixin):
    ...
    def __init__(self, control_mode=0,throttle = 0,steering = 0, brake = 0,
        handbrake = False, is_manual_gear = False, manual_gear = 0,
        ...
        self.is_manual_gear = is_manual_gear
        self.manual_gear = manual_gear
        self.linearvl=linearvl
        self.linearvr=linearvr
...
class MWControls(MsgpackMixin):
    ...
    drive_torque=0.0
    def __init__(self, control_mode=0,throttle = 0,steering = 0, brake = 0,
        handbrake = False, is_manual_gear = False, manual_gear = 0,turning_angle=0,drive_torque=0):
        self.control_mode = control_mode
        self.throttle = throttle
        self.steering = steering
        self.brake = brake
...
class APControls(MsgpackMixin):
    ...
    swing_angle = 0
    rotation_angle = 0
    propeller_throttle = 0
    def __init__(self, control_mode=0,throttle = 0,steering = 0, brake = 0,
        handbrake = False, is_manual_gear = False, manual_gear = 0,
        swing_angle=0,rotation_angle=0,propeller_throttle=0):
        self.control_mode = control_mode
        self.throttle = throttle
        self.steering = steering
        self.brake = brake
        ...
        self.propeller_throttle=propeller_throttle
...
class MWState(MsgpackMixin):
    speed = 0.0
    wheels_hop= 0.0
    wheels_angular_vel= 0.0
    wheels_angular_acc= 0.0
    collision = CollisionInfo()
    kinematics_estimated = KinematicsState()
    timestamp = np.uint64(0)

class APState(MsgpackMixin):
    speed = 0.0
    enginetorque= 0.0
    collision = CollisionInfo()
    kinematics_estimated = KinematicsState()
...
class RadarData(MsgpackMixin):
    point_cloud = 0.0#debug用，整车不应该有point
    data_cloud = 0.0#list of [altitude(°),azimuth(°),depth(m),velocity(m/s)]
    time_stamp = np.uint64(0)
    pose = Pose()
    segmentation = 0
